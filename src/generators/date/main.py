...
from .pytypes import *
from .vars import SEASON_BOUNDS
...
def _resolve_field(
    ...
    if exact is not None:
        if log: print(f"Applying exact_{name} filter: {name} = {exact}")
        ...
        lo = default_min if lo is None else (normalize(lo) if normalize else lo)
        hi = default_max if hi is None else (normalize(hi) if normalize else hi)

        if lo > hi:
            raise ValueError(f"Invalid {name} range: min_{name}={lo} is greater than max_{name}={hi}.")

        if log: print(f"Applying {name} filter: {lo} <= {name} <= {hi}")
        value = randint(int(lo), int(hi))
        if log: print(f"Generated {name}: {value}")
        return value
    # No user args for this field — generate randomly within the caller-supplied defaults.
    if log: print(f"Applying {name} filter: {default_min} <= {name} <= {default_max}")
    value = randint(int(default_min), int(default_max))
    if log: print(f"Generated {name}: {value}")
    return value

def _get_season_int_bounds(season: Season) -> tuple[int, int, int, int]:
...
def gen_date(args: DateArgs, log: bool = False) -> str:
    ...
    before = datetime.strptime(cast(str, before), input_format) if before is not None else None
    after = args.get('after')
    after = datetime.strptime(cast(str, after), input_format) if after is not None else None

    if after is not None and before is not None and after > before:
        raise ValueError("The 'after' date must be earlier than the 'before' date.")
    ...
    def resolve(name, default_min, default_max, normalize=None, normalize_exact=None, clamp_min=None, clamp_max=None):
        ...
        lo = getattr(after, name) if at_after and after is not None else default_min
        hi = getattr(before, name) if at_before and before is not None else default_max
        # Apply any additional clamps (e.g. from season constraints) on top.
        if clamp_min is not None: lo = max(lo, clamp_min)
        if clamp_max is not None: hi = min(hi, clamp_max)
        if lo > hi:
            raise ValueError(f"No valid {name} in [{lo}, {hi}] after applying all constraints.")
        value = int(_resolve_field(args, name, lo, hi, normalize=normalize, normalize_exact=normalize_exact, log=log))
        ...
        at_after = at_after and after is not None and value == getattr(after, name)
        at_before = at_before and before is not None and value == getattr(before, name)
        return value
    ...
    s_start: IntMonthDayPair | None = None  # inclusive lower season bound (month, day)
    s_end:   IntMonthDayPair | None = None  # inclusive upper season bound (month, day)

    if season is not None:
        sm, sd, em, ed = _get_season_int_bounds(season)
        season_str = normalize_season_str(season)
        ...
        if log:
            print(f"Season filter: {season_str} (month {sm} day {sd} → month {em} day {ed})")
        ...
        if wraps:
            ...
            if not can_early and not can_late:
                raise ValueError(f"Season '{season_str}' conflicts with the date range constraints.")
            # If both halves are reachable, pick one randomly; otherwise use whichever fits.
            use_early = choice([True, False]) if (can_early and can_late) else can_early
            if use_early:
            ...
            if log:
                print(f"Winter side: {'early (Jan–Feb)' if use_early else 'late (Dec)'}, month range [{s_clamp_lo}, {s_clamp_hi}]")
        else:
            ...
            if lo_m > em or hi_m < sm:
                raise ValueError(f"Season '{season_str}' (months {sm}–{em}) conflicts with the date range constraints.")
            s_clamp_lo, s_clamp_hi = sm, em
            s_start, s_end = (sm, sd), (em, ed)
            if log:
                print(f"Season month range: [{s_clamp_lo}, {s_clamp_hi}], day bounds: start={s_start}, end={s_end}")

    month = resolve('month', 1, 12, normalize_month_int, normalize_month_int, clamp_min=s_clamp_lo, clamp_max=s_clamp_hi)
    ...
    norm_day = lambda d: normalize_day(year, month, d)
    ...
    if s_start is not None and month == s_start[0]:
        ...
        if log: print(f"Season start-day constraint: lo_day = {lo_day}")
    if s_end is not None and month == s_end[0]:
        ...
        if log: print(f"Season end-day constraint: hi_day = {hi_day}")
    if lo_day > hi_day:
        raise ValueError(f"No valid day in [{lo_day}, {hi_day}] after applying all constraints.")

    day = int(_resolve_field(args, 'day', lo_day, hi_day, normalize=norm_day, normalize_exact=norm_day, log=log))
    ...
    time_zone_data = None
    if before is not None: time_zone_data = before.tzinfo
    elif after is not None: time_zone_data = after.tzinfo
